      ...
      <div className="details-modal publish-modal" onClick={(event) => event.stopPropagation()}>
        ...
        <form className="publish-form" onSubmit={onSubmit}>
          <div className="publish-grid">
            ...
            <label className="form-field">
              <span>Status</span>
              <select value={lostItemForm.status} onChange={(event) => onChange((current) => ({ ...current, status: event.target.value }))}>
          ...
          <div className="publish-grid">
            ...
            <label className="form-field"><span>Contato para retirada</span><input type="text" placeholder="Ex.: secretaria@utp.br ou (41) 99999-1234" value={lostItemForm.contact} onChange={(event) => onChange((current) => ({ ...current, contact: event.target.value }))} /></label>
          </div>
